...
class Simulation():
    ...
    def addHarmonicPolymerBondsSelection(self,
                                         wiggleDist,
                                         bondLength, 
                                         exceptBonds=True):
        """Add harmonic bonds connecting the monomers of a polymer chain.
        Parameters are as above but given as a list for each monomer. 
        This function takes the maximun value of these parameters for each
        pair of connected monomers.
        """

        for start, end, isRing in self.chains:
            for j in range(start, end - 1):
                self.addBond(j, j + 1, max(wiggleDist[j],wiggleDist[j+1]),
                    distance=max(bondLength[j],bondLength[j+1]),
                    bondType="Harmonic", verbose=False)
                if exceptBonds:
                    self.bondsForException.append((j, j + 1))
    ...
    def addPeriodicHarmonicBonds(self,
                            wiggleDist=0.05,
                            bondLength=1.0,
                            pair=4,
                            monomersList=None,
                            exceptBonds=True):
        """Adds harmonic bonds to periodic pairs of monomers
        ...
        particles connected by a bond. True by default.
        pair : integer
        which upfront monomer to bond
        monomersList : array, optional
        specifies which monomers form the helix
        """

        for start, end, isRing in self.chains:
            if isRing:
                print("this potential is not working with a ring polymer", start, end - 1)
            if monomersList is None:
                for j in range(start, end - pair):
                    self.addBond(j, j + pair, wiggleDist,
                                 distance=bondLength,
                                 bondType="Harmonic", verbose=False)
                    if exceptBonds:
                        self.bondsForException.append((j, j + pair))
            else:
                for j in range(len(monomersList) - pair):
                    self.addBond(monomersList[j], monomersList[j + pair], wiggleDist,
                                 distance=bondLength,
                                 bondType="Harmonic", verbose=False)
                    if exceptBonds:
                        self.bondsForException.append((j, j + pair))
        ...
        self.metadata["PeriodicHarmonicBonds"] = repr(
            {"wiggleDist": wiggleDist, 'bondLength':bondLength})
    ...
    def addHandedness(self, theta0, k=1., monomersList=None):
        """Add handedness. k specifies energy in kT at one radian
        If k is an array, it has to be of the length N.
        Xth value then specifies stiffness of the angle centered at
        monomer number X.
        Values for ends of the chain will be simply ignored.
        ...
        Stiffness of the bond.
        If list, then determines stiffness of the bond at monomer i.
        Potential is k * sin(theta) * step(0-sin(theta)) * kT
        """
        try:
            k[0]
        except:
        ...
        handednessForce = self.mm.CustomTorsionForce(
            "kT * torK * step(theta0-theta) ")
        self.forceDict["HandednessForce"] = handednessForce
        for start, end, isRing in self.chains:
            if monomersList is None:
                for j in range(start + 1, end - 1):
                    handednessForce.addTorsion(j - 1, j, j, j + 1, [float(k[j])])
            else:
                for j in range(1,len(monomersList) - 2):
                    handednessForce.addTorsion(int(monomersList[j - 1]), int(monomersList[j]), int(monomersList[j+1]), int(monomersList[j + 2]), [float(k[j])])
            if isRing:
                handednessForce.addTorsion(int(end - 2),int( end - 1), int(start), [float(k[end - 1])  ])
                handednessForce.addTorsion(int(end - 1),int( start), int(start + 1), [float(k[start])  ])

        handednessForce.addGlobalParameter("kT", self.kT)
        handednessForce.addPerTorsionParameter("torK")
        handednessForce.addGlobalParameter("theta0", theta0)
        self.metadata["HandednessForce"] = repr({"handedness": k})
    ...
    def addTestPolynomialRepulsiveForce(self, trunc=3.0, radiusMult=1.):
        """This is a simple polynomial repulsive potential. It has the value
        of `trunc` at zero, stays flat until 0.6-0.7 and then drops to zero
        together with its first derivative at r=1.0.
	...
	"""
        ...
        nbCutOffDist = radius
        repul_energy = (
            "REPe + REPa*rsc12 - REPb*rsc10;"
            #"rsc12 * (rsc2 - 1.0) * REPe / REPemin + REPe;"
            "rsc14 = rsc12 * rsc2;"
            "rsc12 = rsc4 * rsc4 * rsc4;"
	    "rsc10 = rsc4 * rsc4 * rsc2;"
            "rsc4 = rsc2 * rsc2;"
            "rsc2 = rsc * rsc;"
            "rsc = r / REPsigma;")
        self.forceDict["Nonbonded"] = self.mm.CustomNonbondedForce(
        ...
        # repulforceGr.addGlobalParameter('REPrmin', 2.0 / np.sqrt(5.0))
        # Coefficients for x^12*(x*x-1)
        repulforceGr.addGlobalParameter('REPa', 30.0)
        repulforceGr.addGlobalParameter('REPb', 36.0)
        #repulforceGr.addGlobalParameter('REPemin', 46656.0 / 823543.0)
        #repulforceGr.addGlobalParameter('REPrmin', np.sqrt(6.0 / 7.0))
    ...
    def addPolynomialRepulsiveForce_TunableChrom(self, sticky, energy, trunc=3.0, radiusChrom=1., radiusNonChrom=1.):
        """This is a simple polynomial repulsive potential. It has the value
        of `trunc` at zero, stays flat until 0.6-0.7 and then drops to zero
        together with its first derivative at r=1.0.
	    ...
	    """
        radiusnc = self.conlen * radiusNonChrom
        radiusc = self.conlen * radiusChrom
        self.metadata["PolynomialRepulsiveForce_TunableChrom"] = repr({"trunc": trunc})
        nbCutOffDist = radiusNonChrom
        print('trunc=',trunc,'radius=',radiusNonChrom, radiusChrom,'energy=',energy)
        if energy==0:
            print(energy)
            repul_energy = (
                "max(nostick1,nostick2)*(REPe*(1 + 6*rnc14 - 7*rnc12)) + min(sticky1,sticky2)*(REPe + 6*(6*rc14 - 7*rc12));"
                "rc14 = rc12 * rc2;"
                "rc12 = rc4 * rc4 * rc4;"
                "rc4 = rc2 * rc2;"
                "rc2 = rc * rc;"
        ...
        if energy==1:
	        print(energy)
	        repul_energy = (
                "max(nostick1,nostick2)*(REPe*(1 + 6*rnc14 - 7*rnc12)) + min(sticky1,sticky2)*(REPe + 6*(5*rc12 - 6*rc10));"
                "rc12 = rc4 * rc4 * rc4;"
                "rc10 = rc4 * rc4 * rc2;"
                "rc4 = rc2 * rc2;"
                "rc2 = rc * rc;"
        ...
        if energy==2:
            print(energy)
            repul_energy = (
                "max(nostick1,nostick2)*(REPe*(1 + 6*rnc14 - 7*rnc12)) + min(sticky1,sticky2)*(REPe + 6*(4*rc10 - 5*rc8));"
                "rc10 = rc4 * rc4 * rc2;"
                "rc8 = rc4 * rc4;"
                "rc4 = rc2 * rc2;"
                "rc2 = rc * rc;"
        ...
        if energy==3:
            print(energy)
            repul_energy = (
                "max(nostick1,nostick2)*(REPe*(1 + 6*rnc14 - 7*rnc12)) + min(sticky1,sticky2)*(REPe + 6*(3*rc8 - 4*rc6));"
                "rc8 = rc4 * rc4;"
                "rc6 = rc4 * rc2;"
                "rc4 = rc2 * rc2;"
                "rc2 = rc * rc;"
        ...
        if energy==4:
            print(energy)
            repul_energy = (
                "max(nostick1,nostick2)*(REPe*(1 + 6*rnc14 - 7*rnc12)) + min(sticky1,sticky2)*(REPe + 6*(2*rc6 - 3*rc4));"
                "rc6 = rc2 * rc2 * rc2;"
                "rc4 = rc2 * rc2;"
                "rc2 = rc * rc;"
                "rc = r / REPsigmac;"
        ...
        if energy==5:
            print(energy)
            repul_energy = (
                "max(nostick1,nostick2)*(REPe*(1 + 6*rnc14 - 7*rnc12)) + min(sticky1,sticky2)*(step(rc-REPsigmac)*(REPe + 6*(1*rcshift4 - 2*rcshift2))+step(REPsigmac-rc)*(REPe + 6*(1*rc4 - 2*rc2)));"
                "rc4 = rc2 * rc2;"
                "rc2 = rc * rc;"
                "rcshift4 = rcshift2 * rcshift2;"
                "rcshift2 = rcshift * rcshift;"
                "rcshift = r/20 + 0.6;"
                "rc = r / REPsigmac;"
	            "rnc14 = rnc12 * rnc2;"
                "rnc12 = rnc4 * rnc4 * rnc4;"
        ...
        if energy==6:
            print(energy)
            repul_energy = (
                "max(nostick1,nostick2)*(REPe*(1 + 6*rnc14 - 7*rnc12)) + min(sticky1,sticky2)*(step(rc-REPsigmac)*(REPe + 4*(1*rcshift4 - 2*rcshift2))+step(REPsigmac-rc)*(REPe + 4*(1*rc4 - 2*rc2)));"
        	    "rc4 = rc2 * rc2;"
	            "rc2 = rc * rc;"
        	    "rcshift4 = rcshift2 * rcshift2;"
	            "rcshift2 = rcshift * rcshift;"
        ...
        self.forceDict["Nonbonded"] = self.mm.CustomNonbondedForce(
            repul_energy)
        repulforcePolTune = self.forceDict["Nonbonded"]

        repulforcePolTune.addGlobalParameter('REPe', trunc * self.kT)
        repulforcePolTune.addGlobalParameter('REPsigmanc', radiusnc)
        repulforcePolTune.addGlobalParameter('REPsigmac', radiusc)
        # Coefficients for x^8*(x*x-1)
        # repulforceGr.addGlobalParameter('REPemin', 256.0 / 3125.0)
        # repulforceGr.addGlobalParameter('REPrmin', 2.0 / np.sqrt(5.0))
        # Coefficients for x^12*(x*x-1)
        #repulforceGr.addGlobalParameter('REPa', 6.0)
        #repulforceGr.addGlobalParameter('REPb', 7.0)
        repulforcePolTune.addPerParticleParameter('sticky')
        repulforcePolTune.addPerParticleParameter('nostick')

        nostick=[0]*self.N
        for i in range(self.N):
            nostick[i] = 1 - sticky[i]
            repulforcePolTune.addParticle(
                (sticky[i],
                 nostick[i]))

        repulforcePolTune.setCutoffDistance(nbCutOffDist)

    def addPolynomialRepulsiveForceSelection(self, trunc, radius):
        """This is a simple polynomial repulsive potential. It has the value
        of `trunc` at zero, stays flat until 0.6-0.7 and then drops to zero
        together with its first derivative at r=1.0. Applies specific truncation
	and radius values to the list of monomers in selection.
        ...
        nbCutOffDist = max(radius)
        repul_energy = (
            "rsc12 * (rsc2 - 1.0) * max(epsilon1,epsilon2) / REPemin2 + max(epsilon1,epsilon2);"
            "rsc12 = rsc4 * rsc4 * rsc4;"
            "rsc4 = rsc2 * rsc2;"
            "rsc2 = rsc * rsc;"
            "rsc = r / max(sigma1,sigma2) * REPrmin2;")
        repulforcePolSel = self.mm.CustomNonbondedForce(repul_energy)
        repulforcePolSel.setCutoffDistance(nbCutOffDist)
        self.metadata["PolynomialRepulsiveForceSelection"] = repr({"trunc": trunc})

        # Coefficients for x^8*(x*x-1)
        # repulforceGr.addGlobalParameter('REPemin', 256.0 / 3125.0)
        # repulforceGr.addGlobalParameter('REPrmin', 2.0 / np.sqrt(5.0))
        # Coefficients for x^12*(x*x-1)
        repulforcePolSel.addGlobalParameter('REPemin2', 46656.0 / 823543.0)
        repulforcePolSel.addGlobalParameter('REPrmin2', np.sqrt(6.0 / 7.0))

        repulforcePolSel.addPerParticleParameter('epsilon')
        repulforcePolSel.addPerParticleParameter('sigma')

        for i in range(self.N):
            repulforcePolSel.addParticle(
	        (float(trunc[i]),
		 float(radius[i])))

        self.forceDict["Nonbonded"] = repulforcePolSel

    def addPolynomialRepulsiveHeteroSel(self, chrostate):
        """This gives the option for a potential with 24 exponent for heterochromatin,
        as suggested by Fujishiro and Sasai, 2022. Euchromatin is the same as before.
        Chrostate is a list of N ones and zeros to opt for one potential or the other.
        """

        repul_energy = (
            "max(eu1,eu2)*(REPeps*(1 + 6*rse14 - 7*rse12)) + min(het1,het2)*step(1 - rsh)*(REPeps*(1 + 11*rsh24 - 12*rsh22));"
            "rse14 = rse12 * rse2;"
            "rse12 = rse4 * rse4 * rse4;"
            "rse4 = rse2 * rse2;"
            "rse2 = rse * rse;"
            "rse = r / REPsigmaeu;"
            "rsh24 = rsh8 * rsh8 * rsh8;"
            "rsh22 = rsh8 * rsh8 * rsh4 * rsh2;"
            "rsh8 = rsh4 * rsh4;"
            "rsh4 = rsh2 * rsh2;"
            "rsh2 = rsh * rsh;"
            "rsh = r / REPsigmahetero;")
        nbCutOffDist = 10.5 * self.conlen
        repulforceHetSel = self.mm.CustomNonbondedForce(repul_energy)
        repulforceHetSel.setCutoffDistance(nbCutOffDist)

        repulforceHetSel.addGlobalParameter('REPsigmahetero', 8.4)
        repulforceHetSel.addGlobalParameter('REPsigmaeu', 10.5)
        repulforceHetSel.addGlobalParameter('REPeps', 5.)

        repulforceHetSel.addPerParticleParameter('eu')
        repulforceHetSel.addPerParticleParameter('het')
        ...
        for i in range(self.N):
            het[i] = chrostate[i]
            eu[i] = 1 - het[i]
            repulforceHetSel.addParticle(
                (eu[i],
                het[i]))

        self.forceDict["Nonbonded"] = repulforceHetSel

    def addPolynomialRepulsiveCenSel1(self, censtate):
        """This gives the option for a potential with minimum at -1 and roots at 9.67584 and 11.0083.
        Censtate is a list of N ones and zeros to opt for one potential or the other.
        """

        repul_energy = (
            "max(nc1,nc2)*step(1 - rs)*(REPeps*(REPtau + 6*rs14 - 7*rs12)) + min(cen1,cen2)*(REPepscen*(REPtaucen + 6*rs14 - 7*rs12));"
            "rs14 = rs12 * rs2;"
            "rs12 = rs4 * rs4 * rs4;"
            "rs4 = rs2 * rs2;"
            "rs2 = rs * rs;"
            "rs = r / REPsigma;")
        nbCutOffDist = 11.0083 * self.conlen
        repulforceCenSel = self.mm.CustomNonbondedForce(repul_energy)
        repulforceCenSel.setCutoffDistance(nbCutOffDist)

        repulforceCenSel.addGlobalParameter('REPtau', 1)
        repulforceCenSel.addGlobalParameter('REPtaucen', 5/7)
        repulforceCenSel.addGlobalParameter('REPeps', 5.)
        repulforceCenSel.addGlobalParameter('REPepscen', 7.)
        repulforceCenSel.addGlobalParameter('REPsigma', 10.5)


        repulforceCenSel.addPerParticleParameter('nc')
        repulforceCenSel.addPerParticleParameter('cen')
        ...
        for i in range(self.N):
            cen[i] = censtate[i]
            nc[i] = 1 - cen[i]
            repulforceCenSel.addParticle(
                (nc[i],
                cen[i]))

        self.forceDict["Nonbonded"] = repulforceCenSel

    def addPolynomialRepulsiveCenSel2(self, censtate):
        """This gives the option for a potential with minimum at -1 and roots at 9.67584 and 11.0083.
        Censtate is a list of N ones and zeros to opt for one potential or the other.
        """
    ...
    def addPolynomialRepulsiveCenSel3(self, censtate):
        """This gives the option for a potential with minimum at -5, trunc at 10 and roots at 9.57875 and 11.0422.
        Censtate is a list of N ones and zeros to opt for one potential or the other.
        """
        ...
        repul_energy = (
            ...
            "rs2 = rs * rs;"
            "rs = r / REPsigma;")
        nbCutOffDist = 11.0422 * self.conlen
        repulforceCenSel = self.mm.CustomNonbondedForce(repul_energy)
        repulforceCenSel.setCutoffDistance(nbCutOffDist)

        repulforceCenSel.addGlobalParameter('REPtau', 1)
        repulforceCenSel.addGlobalParameter('REPtaucen', 10/15)
        repulforceCenSel.addGlobalParameter('REPeps', 5.)
        repulforceCenSel.addGlobalParameter('REPepscen', 15.)
        repulforceCenSel.addGlobalParameter('REPsigma', 10.5)
    ...
    def addPolynomialRepulsiveCenSel4(self, censtate):
        """This gives the option for a potential with minimum at -3, trunc at 8 and roots at 9.70231 and 10.9984.
        Censtate is a list of N ones and zeros to opt for one potential or the other.
        """
        ...
        repul_energy = (
            ...
            "rs2 = rs * rs;"
            "rs = r / REPsigma;")
        nbCutOffDist = 11.9984 * self.conlen
        repulforceCenSel = self.mm.CustomNonbondedForce(repul_energy)
        repulforceCenSel.setCutoffDistance(nbCutOffDist)

        repulforceCenSel.addGlobalParameter('REPtau', 1)
        repulforceCenSel.addGlobalParameter('REPtaucen', 8/11)
        repulforceCenSel.addGlobalParameter('REPeps', 5.)
        repulforceCenSel.addGlobalParameter('REPepscen', 11.)
        repulforceCenSel.addGlobalParameter('REPsigma', 10.5)
    ...
    def addPolynomialRepulsiveCenSel5(self, censtate):
        """This gives the option for a potential with minimum at -2, trunc at 1 and roots at 8.79649 and 11.2186.
        Censtate is a list of N ones and zeros to opt for one potential or the other.
        """
        ...
        repul_energy = (
            ...
            "rs2 = rs * rs;"
            "rs = r / REPsigma;")
        nbCutOffDist = 11.2186 * self.conlen
        repulforceCenSel = self.mm.CustomNonbondedForce(repul_energy)
        repulforceCenSel.setCutoffDistance(nbCutOffDist)

        repulforceCenSel.addGlobalParameter('REPtau', 1)
        repulforceCenSel.addGlobalParameter('REPtaucen', 1/3)
        repulforceCenSel.addGlobalParameter('REPeps', 5.)
        repulforceCenSel.addGlobalParameter('REPepscen', 3.)
        repulforceCenSel.addGlobalParameter('REPsigma', 10.5)
    ...
    def addPolynomialRepulsiveCenSel6(self, censtate):
        """This gives the option for a potential with minimum at -1, trunc at 4 and roots at 9.85245 and 10.9366.
        Censtate is a list of N ones and zeros to opt for one potential or the other.
        """
        ...
        repul_energy = (
            ...
            "rs2 = rs * rs;"
            "rs = r / REPsigma;")
        nbCutOffDist = 10.9366 * self.conlen
        repulforceCenSel = self.mm.CustomNonbondedForce(repul_energy)
        repulforceCenSel.setCutoffDistance(nbCutOffDist)

        repulforceCenSel.addGlobalParameter('REPtau', 1)
        repulforceCenSel.addGlobalParameter('REPtaucen', 4/5)
        repulforceCenSel.addGlobalParameter('REPeps', 5.)
        repulforceCenSel.addGlobalParameter('REPepscen', 5.)
        repulforceCenSel.addGlobalParameter('REPsigma', 10.5)
    ...
    def addPolynomialRepulsiveCenSel7(self, censtate):
        """This gives the option for a potential with minimum at -0, trunc at 5 and roots at 10.5.
        But with a polynomial functio in the form 5(1+rs^4-2rs^2)
        Censtate is a list of N ones and zeros to opt for one potential or the other.
        """

        repul_energy = (
            "max(nc1,nc2)*step(1 - rs)*(REPeps*(REPtau + 6*rs14 - 7*rs12)) + min(cen1,cen2)*(REPepscen*(REPtaucen + 1*rs4 - 2*rs2));"
            "rs14 = rs12 * rs2;"
            "rs12 = rs4 * rs4 * rs4;"
            "rs4 = rs2 * rs2;"
            "rs2 = rs * rs;"
        ...
        repulforceCenSel.addGlobalParameter('REPtau', 1)
        repulforceCenSel.addGlobalParameter('REPtaucen', 1)
        repulforceCenSel.addGlobalParameter('REPeps', 5.)
        repulforceCenSel.addGlobalParameter('REPepscen', 5.)
        repulforceCenSel.addGlobalParameter('REPsigma', 10.5)
    ...
    def addPolynomialRepulsiveCenSel8(self, censtate):
        """This gives the option for a potential with minimum at -0, trunc at 100 and roots at 10.5.
        Censtate is a list of N ones and zeros to opt for one potential or the other.
        """
        ...
        repulforceCenSel.addGlobalParameter('REPtaucen', 1)
        repulforceCenSel.addGlobalParameter('REPeps', 5.)
        repulforceCenSel.addGlobalParameter('REPepscen', 100.)
        repulforceCenSel.addGlobalParameter('REPsigma', 10.5)
    ...
    def addPolynomialRepulsiveCenSel9(self, censtate):
        """This gives the option for a harmonic potential around 10.5.
        Censtate is a list of N ones and zeros to opt for one potential or the other.
        """

        repul_energy = (
            "max(nc1,nc2)*step(1 - rs)*(REPeps*(REPtau + 6*rs14 - 7*rs12)) + min(cen1,cen2)*(REPepscen*(r-REPsigma)*(r-REPsigma));"
            "rs14 = rs12 * rs2;"
            "rs12 = rs4 * rs4 * rs4;"
            "rs4 = rs2 * rs2;"
            "rs2 = rs * rs;"
        ...
        repulforceCenSel.addGlobalParameter('REPtaucen', 1)
        repulforceCenSel.addGlobalParameter('REPeps', 5.)
        repulforceCenSel.addGlobalParameter('REPepscen', 1.)
        repulforceCenSel.addGlobalParameter('REPsigma', 10.5)
    ...
    def addPolynomialRepulsiveCenSel11(self, censtate):
        """This gives the option for a potential with minimum at -0, trunc at 10 and roots at 10.5.
        Censtate is a list of N ones and zeros to opt for one potential or the other.
        """
        ...
        repulforceCenSel.addGlobalParameter('REPtaucen', 1)
        repulforceCenSel.addGlobalParameter('REPeps', 5.)
        repulforceCenSel.addGlobalParameter('REPepscen', 10.)
        repulforceCenSel.addGlobalParameter('REPsigma', 10.5)
    ...
    def addPolynomialRepulsiveCenSel12(self, chrostate):
        """This gives the option for a potential with 18 exponent for heterochromatin,
        as suggested by Fujishiro and Sasai, 2022. Euchromatin is the same as before.
        Chrostate is a list of N ones and zeros to opt for one potential or the other.
        """
        
        repul_energy = (
            "max(eu1,eu2)*(REPeps*(1 + 6*rse14 - 7*rse12)) + min(het1,het2)*step(1 - rsh)*(REPeps*(1 + 8*rsh18 - 9*rsh16));"
            "rse14 = rse12 * rse2;"
            "rse12 = rse4 * rse4 * rse4;"
            "rse4 = rse2 * rse2;"
            "rse2 = rse * rse;"
        ...
        repulforceHetSel.addGlobalParameter('REPsigmahetero', 9.5)
        repulforceHetSel.addGlobalParameter('REPsigmaeu', 10.5)
        repulforceHetSel.addGlobalParameter('REPeps', 5.)
    ...
    def addPolynomialRepulsiveCenSel13(self, chrostate):
        """This gives the option for a potential with 20 exponent for heterochromatin,
        as suggested by Fujishiro and Sasai, 2022. Euchromatin is the same as before.
        Chrostate is a list of N ones and zeros to opt for one potential or the other.
        """
        
        repul_energy = (
            "max(eu1,eu2)*(REPeps*(1 + 6*rse14 - 7*rse12)) + min(het1,het2)*step(1 - rsh)*(REPeps*(1 + 9*rsh20 - 10*rsh18));"
            "rse14 = rse12 * rse2;"
            "rse12 = rse4 * rse4 * rse4;"
            "rse4 = rse2 * rse2;"
            "rse2 = rse * rse;"
            "rse = r / REPsigmaeu;"
            "rsh20 = rsh8 * rsh8 * rsh4;"
            "rsh18 = rsh8 * rsh8 * rsh2;"
            "rsh8 = rsh4 * rsh4;"
            "rsh4 = rsh2 * rsh2;"
            "rsh2 = rsh * rsh;"
            "rsh = r / REPsigmahetero;")
    ...
    def tetherParticles(self, particles, k=30, positions="current"):
        ...
        if positions == "current":
            ...
            datasel = [self.data[i] for i in particles]
            com = np.sum(datasel,axis=0)/len(particles)
            positions = [com for i in particles]
        else:
